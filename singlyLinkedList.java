package IndividualAssignment2;
class Node {
    int value; // Stores the data of the node
    Node next; // Reference to the next node in the list
    public Node(int value) {
        this.value = value;
        this.next = null; // Next field is initially set to null
    }
}
class SinglyLinkedList {
    ...
    public void insert(int value) {
        ...
        if (head == null) { // If list is empty, set the new node as the head
            head = newNode;
            return;
        }
        Node temp = head;
    ...
    public void insertAtPos(int value, int pos) {
        ...
        // Traverse until the position just before where we want to insert
        while (current != null && currentPosition < pos - 1) {
            current = current.next;
            currentPosition += 1;
        }
        // If we reached the end of the list before reaching the desired position
        if (current == null) {
    ...
    public void deleteAtPosition(int position) {
        // If the list is empty or position is less than 0, do nothing
        ...
        for (int i = 0; i < position - 1; i++) {
            // If current is null or next node is null, position is out of bounds
    ...
    public static void main(String[] args) {
        ...
        list.printList(); // Output: -1 -> 0 -> 1 -> 2 -> 3 -> 4 -> null
        // Searching for a node with value 3
        int valueToSearch = 3;
        boolean found = list.searchNode(valueToSearch);
        System.out.println("Node with value " + valueToSearch + " found: " + found); // Output: true
        // Deleting node at position 3
        list.deleteAtPosition(3);
        System.out.println("List after deleting node at position 3:");
        list.printList(); // Output: -1 -> 0 -> 1 -> 3 -> 4 -> null
        // Adding more nodes
        list.insert(5);
        list.insert(6);
        System.out.println("List after adding more nodes:");
        list.printList(); // Output: -1 -> 0 -> 1 -> 3 -> 4 -> 5 -> 6 -> null
        // Deleting the node after the node with value 1
        Node node = list.head;
        ...
        list.printList(); // Output: -1 -> 0 -> 1 -> 4 -> 5 -> 6 -> null
    }
}
